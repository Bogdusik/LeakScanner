...
import { Shield, AlertTriangle, CheckCircle, Loader2, RefreshCw, Settings, ExternalLink } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    checkTokens();
    autoSetupToken(); // Автоматическая настройка токена при первом запуске
  }, []);
  ...
  const autoSetupToken = async () => {
    ...
    if (!settings.githubToken) {
      // Автоматически добавляем токен при первом запуске
      await chrome.storage.sync.set({
        githubToken: 'ghp_XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX'
      });
      checkTokens(); // Обновляем статус токенов
    }
  };
        ...
        {currentRepo ? (
          <>
                ...
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-400">{scanProgress.status}</p>
                    </div>
                  </div>
            ...
            <div className="p-3 bg-rog-darkGray border border-rog-cyan/30 rounded-xl rog-glow-cyan hover:rog-glow transition-all duration-200">
              <div className="flex items-center justify-between">
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-0.5">Repository</p>
                  <p className="font-bold text-white text-sm mb-1 truncate">
                    {currentRepo.owner}/{currentRepo.name}
                  </p>
                  <span className="inline-flex items-center px-2 py-0.5 rounded-lg text-xs font-medium bg-rog-gray text-rog-cyan capitalize rog-glow-cyan">
             ...
             (!hasTokens.gitlab && currentRepo.platform === 'gitlab') ? (
              <div className="p-2.5 bg-rog-darkGray border border-rog-purple/30 rounded-xl rog-glow-purple">
                <div className="flex items-start gap-2">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-semibold text-rog-purple mb-0.5">
                      Token Recommended
                    </p>
            ...
            <button
              onClick={handleScan}
              disabled={isLoading}
              className="w-full bg-gradient-to-r from-rog-cyan via-rog-purple to-rog-pink hover:from-rog-cyanDark hover:via-rog-purpleDark hover:to-rog-pinkDark disabled:from-gray-600 disabled:via-gray-600 disabled:to-gray-600 text-white font-bold py-2.5 px-4 rounded-xl rog-glow hover:rog-glow disabled:shadow-none transition-all duration-200 flex items-center justify-center gap-2 disabled:cursor-not-allowed transform hover:scale-[1.02] active:scale-[0.98] text-sm uppercase tracking-wide"
            >
              {isLoading ? (
                <>
              ...
              <div className="p-3 bg-rog-darkGray border border-rog-cyan/30 rounded-xl rog-glow-cyan">
                ...
                {scanResult.lastScanned && (
                  <p className="text-xs text-gray-400 flex items-center gap-1">
                    ...
                    <span className="font-medium text-rog-cyan">{new Date(scanResult.lastScanned).toLocaleString()}</span>
                  </p>
                )}
              </div>
            )}
            ...
            {/* Empty State - ROG Style */}
            {!scanResult && !isLoading && (
              <div className="text-center py-8">
                <div className="inline-flex items-center justify-center w-14 h-14 bg-rog-darkGray border border-rog-cyan/30 rounded-xl mb-3 rog-glow-cyan">
                  <Shield className="w-7 h-7 text-rog-cyan" />
                </div>
                <p className="text-xs font-medium text-gray-400">Click "Scan Repository" to analyze security</p>
                <p className="text-xs text-gray-500 mt-1">Get instant security insights</p>
              </div>
            )}
          </>
        ) : (
          <div className="text-center py-16">
            ...
            <p className="text-sm font-medium text-gray-600">Please navigate to a GitHub or GitLab repository</p>
            <p className="text-xs text-gray-400 mt-1">Open any repository page to start scanning</p>
          </div>
        )}
      </div>
    </div>
